...
เจ้าของโปรเจกต์เรียกเวอร์ชันด้วยชื่อสั้น (V4 ถึง V8) เวลาสั่งงาน ไม่ได้เรียกด้วย commit hash

| ref | คืออะไร | commit |
|---|---|---|
| `v8` | **ตัวที่ deploy อยู่ตอนนี้** — ยกเลิกนโยบายรับประกัน 7 วันจริงใน /refund และ /terms + หน้ากฎหมายใช้โลโก้ QVX จริง | `a529084` |
| `v7` | ถอดคำรับประกันคืนเงิน 7 วันออกจากทุกจุดที่โฆษณา (ยังไม่แตะนโยบาย) | `f138cb5` |
| `v6` | หัวหน้าเว็บสองคอลัมน์ (การ์ด + บล็อกโปร) พาดหัวไทย คลิป Hero เข้มเท่าภาพนิ่ง | `138f64d` |
| `v5` | V4 + ภาพชุด SMC / Gold / ICT พื้นหลังเข้มเท่าภาพในฟีเจอร์ | `0ecff3a` |
...
## V8 มีอะไรบ้าง (นับจาก V7)

V7 หยุดโฆษณา — **V8 แก้ตัวนโยบายจริง** เขียนหน้า `/refund` ใหม่ทั้งหน้า
และแก้บทสรุปใน `/terms`

- ตัดสิทธิ์ "เปลี่ยนใจแล้วขอคืนได้ใน 7 วัน" ออก เหตุผลที่ระบุคือเป็นบริการดิจิทัล
  ที่ส่งมอบทันทีและเรียกคืนสิ่งที่ใช้ไปแล้วไม่ได้
- เพิ่มข้อ 1 "ก่อนตัดสินใจชำระเงิน" ชี้ไปตัวอย่างการทำงาน คู่มือ และการถามก่อนซื้อ
  เพราะพอไม่มีระยะทดลอง ภาระการให้ข้อมูลก่อนขายหนักขึ้น
- คงกรณีคืนเงินที่เป็น "เราส่งมอบไม่ได้" ไว้ครบ + เพิ่มกรณีปิดบริการถาวรกลางอายุ
  สมาชิก คืนตามสัดส่วน
- เพิ่มข้อ 9 ระบุว่าการแก้นโยบายในอนาคตมีผลไปข้างหน้าเท่านั้น
- `REFUND_DAYS` -> `LEGACY_REFUND_DAYS` + `REFUND_POLICY_EFFECTIVE`
- ลบ `components/marketing/GuaranteeBadge.tsx`
- `LegalShell` ใช้โลโก้ QVX จริงแทนตัวอักษรในกล่องเขียว (กระทบ /refund /terms /privacy)
  — ของค้างจากตอนเพิ่มโลโก้เข้า Navbar/Footer แล้วมองข้ามหน้ากฎหมายไป

(!) **ห้ามลบหน้า `/refund` และห้ามถอดข้อ 4**

หน้า `/refund` ต้องมีอยู่ เพราะการขายออนไลน์ต้องเปิดเผยเงื่อนไขคืนเงินก่อนชำระเงิน
"ไม่มีนโยบาย" แย่กว่า "มีนโยบายที่จำกัด"

ข้อ 4 คุ้มครองสมาชิกที่จ่ายเงินก่อน 29 สิงหาคม 2026 ซึ่งตอนนั้นเว็บโฆษณาการรับประกัน
ไว้จริง เขาได้รับคำมั่นนั้นไปแล้ว ถอดข้อนี้ออกเท่ากับใช้เงื่อนไขใหม่ย้อนหลัง
สำเนาหน้าเวอร์ชันที่ยังโฆษณาอยู่: `git show v6:src/app/refund/page.tsx`

หมายเหตุ: เนื้อหากฎหมายเขียนตามหลักที่สมเหตุสมผล แต่ยังไม่ผ่านการตรวจโดยผู้รู้
ด้านกฎหมายคุ้มครองผู้บริโภค ควรให้ตรวจทวนเมื่อยอดขายเริ่มมาก

## V7 มีอะไรบ้าง (นับจาก V6)

ถอดคำรับประกันคืนเงิน 7 วันออกจาก **ทุกจุดที่โฆษณา** รวม 6 จุด:

- แถบใหญ่ใต้บล็อกโปรในหน้าราคา (`GuaranteeStrip`)
- บรรทัด "คืนเงินได้ใน 7 วัน" ใต้ปุ่มสมัครของทุกแพ็กเกจ 4 ใบ (`GuaranteeLine`)
- บรรทัดเดียวกันในหน้าเลือกแพ็กเกจของพอร์ทัลสมาชิก (`account/subscription`)

`components/marketing/GuaranteeBadge.tsx` จึงไม่มีใครเรียกทั้งไฟล์ แต่ยังไม่ลบทิ้ง

สิ่งที่ยังอยู่ในหน้าราคา และตั้งใจให้อยู่:

- "การสมัครสมาชิกไม่รับประกันผลกำไร" — คำเตือนความเสี่ยง คนละเรื่องกับการคืนเงิน
- ลิงก์ "ตรวจสอบรายละเอียดการคืนเงิน ก่อนชำระเงิน" — เปิดเผยนโยบายก่อนจ่าย
  ไม่ใช่การให้สัญญา และเป็นแนวปฏิบัติที่ควรมีตามกฎหมายคุ้มครองผู้บริโภค

(!) **นี่คือหยุดโฆษณา ไม่ใช่ยกเลิกนโยบาย** — `/refund` (6 จุด) กับ `/terms`
ยังเขียนสัญญา 7 วันอยู่ ซึ่งเป็นเอกสารที่ผูกพันจริง เว็บจึงอยู่ในสถานะ
"ให้จริงมากกว่าที่โฆษณา" ซึ่งปลอดภัยกับลูกค้า
ถ้าจะยกเลิกจริงต้องแก้พร้อมกันสามที่: `REFUND_DAYS` ใน `config/legal.ts`,
เนื้อหา `/refund`, เนื้อหา `/terms` — และสมาชิกที่สมัครก่อนหน้านั้นยังได้สิทธิ์
ตามเงื่อนไขเดิม สำเนาหน้า `/refund` ตอนที่ยังสัญญาไว้อยู่ที่ tag `v6`

## V6 มีอะไรบ้าง (นับจาก V5)

หัวหน้าเว็บเป็นสองคอลัมน์ 7:5 บนจอ `lg` ขึ้นไป — การ์ดสรุปสินค้าซ้าย บล็อกโปรขวา
ลูกค้าขอให้ "เปิดมาหน้าแรกเจอเลย" ของเดิมบล็อกโปรอยู่ใต้การ์ดซึ่งสูง ~950px
จอแคบกว่านั้นยังเรียงบนล่าง เพราะบีบสองคอลัมน์แล้วกราฟเล็กจนอ่านไม่ออก

- ราคาโปร (ราคาปกติขีดฆ่า -> 999) โชว์ในบล็อกโปรบนหน้าแรกด้วย ไม่ใช่เฉพาะหน้าราคา
- หัวข้อในบล็อกโปรใหญ่ขึ้นหนึ่งขั้น รายละเอียดย่อลงเหลือ 11px
- ขอบบล็อกโปรสว่างขึ้น 30% -> 62% ของสีแบรนด์ (ใบเดียวในเว็บที่ได้ระดับนี้)
- คลิป `qvx-hero-v2.mp4` พื้นหลังเข้มเท่าภาพนิ่ง — ตอนนี้ทุกสื่อในเว็บอยู่ที่ RGB 8

(!) **พาดหัวเปลี่ยนเป็นไทย** `["วิเคราะห์ตลาด", "ในระบบเดียว"]` ไม่ใช่แค่รสนิยม —
พอการ์ดเหลือกว้าง ~580px ในคอลัมน์ "Complete XAUUSD Analysis" ตกเป็นสามบรรทัด
โดยตัดกลางวลี ถ้าจะย้อนเป็นอังกฤษต้องลดขนาดพาดหัวใน `ProductCard` ด้วย
(รายละเอียดอยู่ในคอมเมนต์ที่ `productCard` ใน `config/features.ts`)

> หมายเหตุ: ชื่อ V4 / V5 / V6 หมายถึง "หน้าตาเว็บ" แต่ tag จับสภาพทั้ง repo ณ ตอนนั้น
> ซึ่งมีงานหลังบ้านของอีกสายงาน (บอท TradingView, ตัวไล่งานค้าง, กันสลิปซ้ำ) ติดมาด้วย
> ย้อนหน้าตาอย่างเดียวให้ใช้ `git checkout <tag> -- <ไฟล์ที่ต้องการ>` ทีละไฟล์ ไม่ใช่ทั้ง tag
