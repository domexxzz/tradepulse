...
> ✅ ติดตั้ง patch ฝั่งบอทแล้ว 31 ส.ค. 2569 (`botpatch.py` บนคอมเฟิร์ส) บริดจ์รีสตาร์ตสะอาด
> ⚠️ แต่ **ยังไม่เคยทดสอบ grant จริงครบวง** — ยังไม่รู้ว่าภาพไปถึง DM ลูกค้าจริงไหม
> (ดูหัวข้อ "สถานะล่าสุด — 26 ก.ย. 2569" ท้ายไฟล์)
...
## สถานะล่าสุด — 26 ก.ย. 2569

**งานเรื่องบริดจ์ยังไม่ขยับตั้งแต่ 31 ส.ค.** — วันถัดมาไดรฟ์ D: ของเครื่อง dev เสีย
(อ่านได้อย่างเดียว) ตอนนี้ทำงานต่อที่ `E:\d-rescue\WebPeeNOT`
ส่วนงานด้านอื่นเดินต่อแล้ว (ช่องทาง Wisdom · API สิทธิ์ · โครง Discord — 26 ก.ย.)
วิธีเปลี่ยนบัญชี TradingView ของบอท และค่าลับที่ต้องเปลี่ยนเมื่อหลุด ดู `docs/MIGRATE.md`

### ทำอะไรเพิ่มหลังหัวข้อ "ภาพหลักฐานสิทธิ์"

ห้าคอมมิต ทั้งหมดวันที่ 31 ส.ค. 2569

| คอมมิต | ทำอะไร |
|---|---|
| `f2e9433` | วาง `tvshot.py` ใน `public/` ให้เครื่องบอทโหลดเอง |
| `2d1eaf7` | `botpeek.py` — ส่องโครงสร้างไฟล์บนเครื่องบอทก่อนเขียน patch |
| `93a3712` | `botpatch.py` — แก้ `tlapi.py` + `tv_bridge.py` ให้ส่ง proof กลับเว็บ |
| `6cb77e7` | ปุ่มปลดผูกบัญชี Telegram ในหน้าแอดมิน |
| `0a528f3` | แก้ปุ่มปลดผูกให้ยึดกับ**สมาชิก** ไม่ใช่แถวคิว (ดู `docs/TELEGRAM.md`) |

### ✅ ติดตั้งบนเครื่องบอทแล้ว — แต่ยังไม่เคยทดสอบครบวง

บันทึกของเซสชันที่ลงมือทำ (31 ส.ค. 2569) ระบุว่า

- `tvshot.py` ลงเครื่องคอมเฟิร์สแล้ว (`C:\BotTV\`)
- `botpatch.py` แทรกโค้ดใน `tlapi.py` แล้ว — import แบบ try, ตัวแปร
  `LAST_PROOF = {"username","b64"}` และบล็อกแคปภาพหลัง `EXPIRY_OK` (บรรทัด 567)
- แทรกใน `tv_bridge.py` แล้ว — เติม `import tlapi` และใส่ `"proof"` ต่อจาก
  `"error": error,` (บรรทัด 166)
- สำรองไฟล์เดิมเป็น `.bak-proof` ไว้ทั้งคู่
- รีสตาร์ตบริดจ์แล้วขึ้นสะอาด ไม่มี Traceback

**กันภาพหลุดไปผิดคนด้วยโครงสร้าง** — ฝั่ง bridge อ่านภาพด้วย `pop`
และต้อง username ตรงกับคนที่กำลังส่ง callback เท่านั้น
จึงส่งซ้ำไม่ได้ ส่งข้ามคนไม่ได้ รอบที่แคปพลาดได้ `None`

**สิ่งที่ยังไม่ได้ทำ:** ยังไม่เคยสั่ง grant จริงแล้วดูว่าภาพไปถึง DM ลูกค้า

> ℹ️ สถานะฝั่งเครื่องบอทดูจากรีโปนี้ไม่ได้ — โค้ดบอทอยู่คนละที่ ต้องเช็กบนเครื่องบอทเอง

**เช็กว่ายังติดตั้งอยู่ไหม** (บนเครื่องบอท)

```powershell
findstr PROOF_V1 C:\BotTV\tlapi.py C:\BotTV\tv_bridge.py
dir C:\BotTV\*.bak-proof
```

**ทดสอบครบวง** — สั่ง grant บัญชีทดสอบ แล้วดูค่า `proof` ที่ callback ตอบ
(ตารางค่าอยู่หัวข้อ "สัญญาการเรียก") ถ้าได้ `sent` = ภาพถึง DM แล้ว

> ⚠️ อินดิเคเตอร์ที่บอทจัดการชื่อ **"Test 2 SMC Unified Suite"** —
> ยังไม่ได้ยืนยันว่าเป็นตัวจริงที่ลูกค้าต้องได้ ควรเช็กก่อนเปิดขายจริง

### วิธีติดตั้ง patch บนเครื่องบอท

สคริปต์ทั้งสามวางไว้ใน `public/` ของเว็บ เครื่องบอทโหลดเองผ่าน HTTPS
(วิธีเดียวกับที่เคยใช้กับ `botpatch2.py` รอบก่อน)

```powershell
cd C:\BotTV
curl.exe -o tvshot.py   https://quantvisionx.com/tvshot.py
curl.exe -o botpatch.py https://quantvisionx.com/botpatch.py
python botpatch.py
```

`botpatch.py` กันพังไว้สามชั้น

- สำรองไฟล์เดิมเป็น `.bak-proof` ก่อนเสมอ และไม่ทับของเก่าถ้ามีอยู่แล้ว
- `ast.parse` ตรวจก่อนเขียนทับ ถ้า python อ่านไม่ออกจะไม่แตะไฟล์นั้นเลย
- รันซ้ำได้ เจอ marker `PROOF_V1` แล้วจะข้าม ไม่แทรกซ้ำ

ยึดจุดแทรกด้วย **ข้อความในโค้ด** ไม่ใช่เลขบรรทัด และบังคับว่าต้องเจอครั้งเดียวเท่านั้น
เจอศูนย์ครั้งหรือหลายครั้ง = ไม่แตะไฟล์นั้น

> `print` ในสคริปต์เป็น ASCII ล้วนโดยตั้งใจ — คอนโซลเครื่องบอทเป็น cp874 พิมพ์ไทยแล้วเคยตาย

### 🧹 ของค้างที่ยังไม่ได้เก็บ

`public/botpatch.py` · `public/botpeek.py` · `public/tvshot.py` — **ยังอยู่จนถึงวันนี้**

ทั้งสามตั้งใจวางไว้ชั่วคราวให้เครื่องบอทโหลด แต่ทุกไฟล์ใน `public/` เปิดให้ใครก็โหลดได้

ตรวจแล้ว **ไม่มี secret หรือ token ติดไปในทั้งสามไฟล์** (`botpeek.py` มีตัวเซ็นเซอร์
สตริงยาวในตัวอยู่แล้ว) จึงไม่ใช่เรื่องด่วน แต่มันเปิดเผยโครงสร้างไฟล์และวิธีทำงาน
ของเครื่องบอทให้คนนอกเห็น — **ลบได้ทันทีที่ patch ติดตั้งเสร็จ**

`botpatch2.py` กับ `botpatch2.ps1` ของรอบก่อนถูกลบไปแล้ว
