 ...
 *
 * คลิป Hero เป็นคนละไฟล์ ยังใช้อยู่บนหน้าแรก น็อตอัดใหม่ให้ที่ 5634x2160 เมื่อ 30 ส.ค. 2026
 * ย่อลงเหลือ 1772x680 ไม่ต้องครอปเพราะไม่มี UI ของแอปติดมา
 * (ประวัติการเข้ารหัสทั้งสามรอบอยู่ที่ heroClip ท้ายไฟล์)
 *
 ...
 *
 * ── รอบสาม (-v3) 30 ส.ค. 2026 ────────────────────────────────────────────
 *
 * น็อตอัดคลิปใหม่มาให้ที่ 5634x2160 60fps (เดิม 736px) ซึ่งแก้ข้อจำกัดที่เขียนไว้
 * ข้างบนได้หมด — ไม่ต้องขยายอีกแล้ว กลับกันคือย่อลง 3.2 เท่า รายละเอียดจึงเป็นของจริง
 * ไม่ใช่ที่สร้างขึ้นจาก unsharp และคลิปนี้ไม่มีลายน้ำ "การเล่นซ้ำ" กลางจอแบบตัวเก่า
 *
 * เลือกช่วง 20-60 วินาที เพราะเป็น Bar Replay ที่ค่อย ๆ เดิน ช่วงต้นกราฟยังว่าง
 * วัดสัดส่วนพิกเซลที่มีเนื้อหาแล้วหนาแน่นสุดช่วง 25-45 วินาที
 *
 * ลดจาก 60fps เหลือ 30 — เป็นคลิปพื้นหลังวนซ้ำ ไม่มีใครจ้องความลื่น
 * แลกมาด้วยบิตเรตครึ่งเดียว และตัดเสียงทิ้งเพราะ LoopingClip เล่นแบบ muted อยู่แล้ว
 *
 * ขนาด 1772x680 = 2 เท่าของช่องจริงบน Hero (886px วัดจากเบราว์เซอร์)
 * ผลลัพธ์ 1.8 MB เล็กกว่าตัวเก่าที่ 2.6 MB ทั้งที่ความละเอียดสูงกว่า
 *
 * ffmpeg -ss 20 -t 40 -i <ต้นฉบับจากน็อต> \
 *   -vf "scale=1772:680:flags=lanczos,curves=all='0/0 0.06/0.034 0.25/0.24 1/1'" \
 *   -r 30 -c:v libx264 -preset slow -crf 23 -pix_fmt yuv420p -an -movflags +faststart \
 *   qvx-hero-v3.mp4
 *
 * poster ดึงเฟรมแรกจากไฟล์ที่เข้ารหัสเสร็จแล้ว เฟรมจึงตรงกับตอนวิดีโอเริ่มเป๊ะ
 *   ffmpeg -i qvx-hero-v3.mp4 -frames:v 1 -c:v libwebp -quality 88 qvx-hero-v3.webp
 */
export const heroClip: GuideVideo = {
  src: "/videos/qvx-hero-v3.mp4",
  poster: "/images/videos/qvx-hero-v3.webp",
